...
def model_train(net, partition, optimizer, scaler, epoch, num_classes, args):
    ...
    for i, data in enumerate(trainloader,0):
        ...
        ### for BMI regression
        labels_txt = ["{}".format(int(round(value,0))) for value in labels_txt]
        prompt = "You are a neurologist and now you are analyzing T1-weighted MRI images. Question: Analyze the image and estimate the BMI of a person. Answer with number only without any additional text. Answer: "
        ### for weight classification 
        #prompt = "You are a neurologist and now you are analyzing T1-weighted brain MRI images. Question: Analyze the image and estimate the weight status of the children. Answer with 'underweight', 'normal', or 'overweight'. Answer: "
        ...
        if args.use_amp: 
            ...
            try:
                ...
                #acc = accuracy_score(labels.tolist(), pred_num)
                ### for regression
                #acc = nn.functional.mse_loss(torch.tensor(pred), torch.tensor([float(value) for value in batch['text_output']])).item()
                #acc = r2_score(torch.tensor([int(value.split(" ")[0]) for value in batch['text_output']]), torch.tensor([int(value.split(" ")[0]) for value in pred]))
                acc = r2_score(torch.tensor([int(round(value,0)) for value in batch['text_output']]), torch.tensor(pred))
...
def model_validation(net, partition, epoch, num_classes, args):
    ...
    with torch.no_grad():
        for i, data in enumerate(valloader,0):
            ...
            #prompt = "You are a neurologist and now you are analyzing T1-weighted brain MRI images. Question:  Analyze the image and estimate sex of subject from this image. Answer with 'male' or 'female'. Answer: "
            ### for age regression
            #labels_txt = ["{} years-old".format(int(value)) for value in labels_txt]
            #prompt = "You are a neurologist and now you are analyzing T1-weighted brain MRI images. Question: Analyze the image and estimate the age of a person. Answer with number only without any additional text. Answer: "
            ### for BMI regression
            labels_txt = ["{}".format(int(round(value,0))) for value in labels_txt]
            prompt = "You are a neurologist and now you are analyzing T1-weighted brain MRI images. Question: Analyze the image and estimate the BMI of a person. Answer with number only without any additional text. Answer: "
            ### for weight classification 
            #prompt = "You are a neurologist and now you are analyzing T1-weighted brain MRI images. Question: Analyze the image and estimate the weight status of the children. Answer with 'underweight', 'normal', or 'overweight'. Answer: "
            #labels_txt = labels_txt
