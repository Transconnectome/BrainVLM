...
def model_train(net, partition, optimizer, scaler, epoch, num_classes, args):
    ...
    for i, data in enumerate(trainloader,0):
        ...
        for _cat_target in args.cat_target: 
            performance[_cat_target] = [] 

            labels_txt = labels[_cat_target].tolist()
            labels_txt = ['male' if sex == 0 else 'female' for sex in labels_txt]
            prompt = "You are a neurologist and now you are analyzing T1-weighted brain MRI images. Question:  Analyze the image and estimate sex of subject from this image. Answer with 'male' or 'female'. Answer: "
            ...
            if args.use_amp: 
                ...
                try:
                    ...
                    pred_num = [0 if sex == 'male' else 1 for sex in pred]
                    acc = accuracy_score(labels[_cat_target].tolist(), pred_num)

                except: 
                    acc = np.nan
                    #print("Answers still contain additional text.")
                performance[_cat_target].append(acc)
        ...
        for _num_target in args.num_target: 
            performance[_num_target] = [] 

            labels_txt = labels[_num_target].tolist()
            ### for age regression
            #labels_txt = ["{}".format(int(value)) for value in labels_txt]
            #prompt = "You are a neurologist and now you are analyzing T1-weighted brain MRI images. Question: Analyze the image and estimate the age of a person. Answer with number only without any additional text. Answer: "
            ### for BMI regression
            labels_txt = ["{:.1f}".format(float(value)) for value in labels_txt]
            prompt = "You are a neurologist and now you are analyzing T1-weighted MRI images. Question: Analyze the image and estimate the BMI score of a person. Answer with number only without any additional text. Answer: "
            inst = [prompt for _ in range(len(labels_txt))]
            batch['text_input'] = inst 
            batch['text_output'] = labels_txt  
            if args.use_amp: 
                ...
                try:
                    ### for age regression
                    #r2 = r2_score(torch.tensor([int(value) for value in batch['text_output']]), torch.tensor([int(value) for value in pred]))
                    ### for BMI regression
                    #r2 = r2_score(torch.tensor([float(value) for value in batch['text_output']]), torch.tensor(pred))

                    ### special 
                    abs_err = torch.nn.functional.l1_loss(torch.tensor([float(value) for value in batch['text_output']]), torch.tensor(pred)).item()
                except: 
                    #r2 = np.nan
                    abs_err = np.nan
                    #print("Answers still contain additional text.")
                #performance[_num_target].append(r2)
                performance[_num_target].append(abs_err)
