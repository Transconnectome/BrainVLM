...
def model_train(net, partition, optimizer, scaler, epoch, num_classes, args):
    ...
    accs = []
    performance = {} 
    for t in args.cat_target+args.num_target:
        performance[t] = []
    ...
    for i, data in enumerate(trainloader,0):
        ...
        assert len(args.cat_target) == len(args.num_target)
        assert args.cat_target[0] == 'sex'
        prompt = "You are a neurologist and now you are analyzing T1-weighted brain MRI images. Question:  Analyze the image and estimate sex and BMI score of subject from this image. Answer BMI score as a number only without any additional text. Answer: "
        labels_txt = [] 
        inst = []
        for j, (_cat_label, _num_label) in enumerate(zip(labels[args.cat_target[0]].tolist(), labels[args.num_target[0]].tolist())):
            _cat_label_txt = 'male' if _cat_label == 0 else 'female'
            _num_label_txt = "{:.1f}".format(float(_num_label))
            labels_txt.append(f"sex is {_cat_label_txt} and BMI score is {_num_label_txt}")
            inst.append(prompt)
        ...
        if args.use_amp: 
            ...
            try:
                ...
                for k, (target_txt, pred_txt) in enumerate(zip(batch['text_output'], pred)):
                    target_cat_txt = target_txt.split(" ")[2]
                    _target_cat.append(target_cat_txt)
                    pred_cat_txt = pred_txt.split(" ")[2]
                    _pred_cat.append(pred_cat_txt)
                    target_num_txt = target_txt.split(" ")[-1]
                    _target_num.append(target_num_txt)
                    pred_num_txt = pred_txt.split(" ")[-1]
                    _pred_num.append(pred_num_txt)
                target_cat = [0 if sex == 'male' else 1 for sex in _target_cat]
                pred_cat = [0 if sex == 'male' else 1 for sex in _pred_cat]
                target_num = [float(value) for value in _target_num]
                pred_num = [float(value) for value in _pred_num]
                acc = accuracy_score(target_cat, pred_cat)
                abs_err = torch.nn.functional.l1_loss(torch.tensor(target_num), torch.tensor(pred_num)).item()
                #r2 = r2_score(target_num, pred_num)
            ...
            performance[args.cat_target[0]].append(acc)
            performance[args.num_target[0]].append(abs_err)
            #performance[args.num_target[0]].append(r2)
    ...
    performance_summary = {}
    for i, target in enumerate(performance.keys()): 
        performance_summary[target] = np.nanmean(performance[target])
    
    return net, np.mean(losses), performance_summary
...
def train_experiment(partition, num_classes, save_dir, args): #in_channels,out_dim
    ...
    elif args.model == 'blip2_t5xl_instruct':
        from model.model_Blip_t5_instruct import Brain_BLIP
        net = Brain_BLIP(model_arch="blip2_t5_instruct",
                        model_type="flant5xl",
                        img_size=args.img_size[0],
                        lora_vit=False,
                        lora_llm=False,
    ...
    if args.optim == 'SGD':
        ...
        optimizer = optim.AdamW(net.parameters(), lr=args.lr, weight_decay=args.weight_decay,betas=(0.9, 0.95))
    else:
