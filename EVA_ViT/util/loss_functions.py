...
class loss_forward(torch.nn.Module): 
    ...
    def forward(self, pred_y, true_y):
        ...
        if self.num_classes > 1:
            ...
            loss = self.criterion(pred_y.float(), true_y.unsqueeze(-1).float()) 
        return loss
    ...
    def get_result(self):
        ...
        if self.num_classes > 1: 
            if self.is_DDP:
                ...
                total, correct  = [torch.zeros(self.total.size()).cuda().float() for r in range(self.world_size)], [torch.zeros(self.correct.size()).cuda().float() for r in range(self.world_size)]
                true, pred  = [torch.zeros(self.true.size()).cuda().float() for r in range(self.world_size)], [torch.zeros(self.pred.size()).cuda().float() for r in range(self.world_size)]
                torch.distributed.all_gather(total, self.total.cuda().float())
                torch.distributed.all_gather(correct, self.correct.cuda().float())
                torch.distributed.all_gather(true, self.true.cuda().float())
                torch.distributed.all_gather(pred, self.pred.cuda().float())
