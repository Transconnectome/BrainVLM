...
class Attention(nn.Module):
    def __init__(
        ...
        if use_lora:
            self.qkv = lora.MergedLinear(dim, all_head_dim * 3, r=128, enable_lora=[True, False, True])
        else:
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        if self.use_conv_stem: 
            self.patch_embed_3d = nn.Sequential(
                ...
                nn.Conv3d(embed_dim, embed_dim, kernel_size=3, stride=2, padding=1),
                nn.GELU(),
                nn.MaxPool3d(kernel_size=3, stride=2, padding=1)
            )
            num_patches = 8 * 8 * 8
        ...
        self.fc_head = nn.Linear(embed_dim, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward_features(self, x):
        ...
        if self.use_projector: 
            for proj_blk in self.projectors:
                if self.use_checkpoint:
                    x = checkpoint.checkpoint(proj_blk, x, rel_pos_bias)
                else:
                    x = proj_blk(x, rel_pos_bias)
        return x
...
def create_eva_vit_g(img_size=256,patch_size=16,drop_path_rate=0.4,use_checkpoint=False,precision="fp16", num_classes=None, use_projector=False, use_conv_stem=False,use_mean_pooling=False, use_lora=False, use_pretrained_weight=False):
