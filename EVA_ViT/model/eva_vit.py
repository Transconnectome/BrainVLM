...
class Attention(nn.Module):
    def __init__(
            ...
            proj_drop=0., window_size=None, attn_head_dim=None, use_lora=False):
        super().__init__()
        ...
        if use_lora:
            self.qkv = lora.MergedLinear(dim, all_head_dim * 3, r=8, enable_lora=[True, False, True])
        else:
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 window_size=None, attn_head_dim=None, use_lora=False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, window_size=window_size, attn_head_dim=attn_head_dim, use_lora=use_lora)
...
def create_eva_vit_g(img_size=256,patch_size=28,drop_path_rate=0.4, use_checkpoint=False,  use_lora=False):
...
class Brain_BLIP_image(pl.LightningModule): 
    def __init__(self, config: dict):
        ...
        # setting model
        self.model = create_eva_vit_g(img_size=self.hparams.model.image_encoder.img_size,
    ...
    def training_step(self, batch, batch_idx): 
        os.environ['GLOBAL_RANK'] = str(self.global_rank)
        
        #loss, loss_dict = self.model(batch)
        loss, loss_dict = self(batch['image'])
        self.log_dict({
    ...
    def on_validation_epoch_end(self): 
        ...
        columns = ['text', 'inst', 'answer']
        data = list(zip(text, qna, answer))
    ...
    def configure_optimizers(self): 
        # setting optimizers
        import deepspeed
        optim = deepspeed.ops.adam.DeepSpeedCPUAdam(filter(lambda p: p.requires_grad, self.parameters()), lr= self.learning_rate, weight_decay=self.hparams.training_parameters.weight_decay)
