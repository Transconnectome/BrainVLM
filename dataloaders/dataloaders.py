...
def partition_dataset(imageFiles_labels,args):
    ...
    for imageFile_label in imageFiles_labels:
      ...
      # ---------- undersampling block ---------- #
    if getattr(args, "undersample", False):
        keys = args.cat_target + args.num_target     # 타깃 조합
        tuple_labels = [tuple(lbl[k] for k in keys) for lbl in labels]
        ...
        images  = df_bal["image"].tolist()
        labels  = df_bal["orig_label"].tolist()

        # 분포 확인
        main_tgt = (args.cat_target + args.num_target)[0]
        print("\n[After undersampling] label distribution (train+val+test)")
        print(pd.Series([l[main_tgt] for l in labels]).value_counts().sort_index(), "\n")
    # ---------- undersampling block ---------- #
