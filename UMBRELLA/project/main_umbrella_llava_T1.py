...
This module implements the main training script for UMBRELLA using LLaVA architecture
for T1-weighted MRI image analysis.
"""
...
from dataset.dataset_T1_LLaVa import ABCD_T1, UKB_T1
...
def __main__():
    ...
    os.environ['WANDB_PROJECT'] = "UMBRELLA"
    ...
    ### setting rsfMRI dataset (placeholder for future implementation)
    rsfMRI_train_datasets = []
    rsfMRI_eval_datasets = []
    rsfMRI_test_datasets = []
    if len(config.dataset.rsfMRI.study_sample) == len(config.dataset.rsfMRI.img_dir) == 0:
        pass
    else:
        raise NotImplementedError(
            "rsfMRI dataset for LLaVa is not implemented yet. Please check the configuration file and make sure that 'study_sample' and 'img_dir' are set to empty lists if you do not want to use rsfMRI dataset."
        )
    ...
    training_args = TrainingArguments(
        ...
        do_train=True,
        do_eval=True,
        remove_unused_columns=False, # This should set as False so that input dictionary can have 'modality' as one of keys
        # training
    ...
    trainer = CustomTrainer(
        ...
        preprocess_logits_for_metrics=preprocess_logits_for_metrics,    # It should be used for preventing memory leakage during evaluation
