...
# Configuration
SPLITS_DIR = Path("sex_comparison_splits")
OUTPUT_DIR = Path("sex_comparison_conversations")
...
def save_conversations(split_name: str, conversations: List[Dict], output_dir: Path):
    ...
    for conv in conversations:
        conv_id = conv['task_id']
        output_path = split_dir / f"{conv_id}.json"

        with open(output_path, 'w') as f:
            json.dump(conv, f, indent=2)

    # Also save all conversations as a single JSONL file
    jsonl_path = split_dir / f"{split_name}_conversations.jsonl"
